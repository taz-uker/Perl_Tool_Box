...
my @job_names = undef;   # array of job_names from input file of PLAIN OLD job names.
...
open(IN_JN, "$input_file_jn") or die "Problem processing the $input_file_jn, specifically $!";
@job_names = <IN_JN>;
close(IN_JN);
...
if ((scalar @bad_jn) > 0) {
    ...
    say "You will have to clean up the file ";
...
say "  NOTE\:  The output file will ONLY contain fields that are present in the";
...
if ($option eq "n") {
    ...
    say "Please enter the name of the file of the Autosys JIL source file you want to use.";
    chomp($input_file_jil = <STDIN>);
}
elsif ($option eq "y"){
    say "\nYou have decided to keep and use the $workingfile_name as you Autosys JIL source file.";
    $input_file_jil = $workingfile_name;
}
else {
...
open(IN_JIL, "$input_file_jil") or die "Problem processing the $input_file_jil, specifically $!";
@workingfile = <IN_JIL>;
close (IN_JIL);

print "INPUT FILE JIL $input_file_jil\n";


SKIP_JIL_LOAD:

say "I skiped the file jil load!!!\n";
