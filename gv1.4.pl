...
foreach $line (@workingfile) {    
      if ($line =~ /insert_job\:/) { # get job name $jn
        ...
        if ($line =~ /condition\:/) {  # gets condition line  
           ...
           @all_gv_items = grep { /\bv\(/ } @all_conditions; #finds all gv's in line
           # print "Job Name \n $jn\n";
...
print "OUTPUT INDEX\n\n";
print scalar @output_index, "\n\n";

$dummy = <STDIN>;

@uniq_gv_output = uniq(sort(@output_index));

print "Unique line from UNIQ command\n\n";
print scalar @uniq_gv_output, "\n\n";

$dummy = <STDIN>;

my $prev; 

foreach $line (@uniq_gv_output) { 
       ($gv, $jn) = split /\s+/, $line; 
       unless ((defined $line) && ($line eq $prev)) {
              printf "  %5s %25s \n", $gv, $jn; 
              $prev = $line;
       }
}   
